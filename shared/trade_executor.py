from shared.paper_engine import buy, sell
from shared.snapshot import invalidate_market_snapshot_cache
from shared.lot_sizing import format_lot_log, quantity_to_lots
from shared.trade_manager import can_buy
from shared.telegram_bot import send_trade_buy, send_trade_sell
from shared.logger import log_info, log_error


def execute_signal(portfolio, signal, current_price):
    if not signal:
        return
    ...
    if action == "REJECTED":
        log_error(
            f"{portfolio.ai_name.upper()} BUY rejected: "
            f"{signal.get('reason', 'Invalid lot sizing')}"
        )
        return
    ...
    if action == "BUY":
        ...
        allowed, reason = can_buy(portfolio, quantity, current_price)
        if not allowed:
            log_error(f"{portfolio.ai_name.upper()} BUY rejected: {reason}")
            return
